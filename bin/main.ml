...
let handle_event inject =
  let keyboard_handler =
    Keyboard_event_handler.of_command_list_exn
      [ { Command.keys = [ Keystroke.create' Js_of_ocaml.Dom_html.Keyboard_code.ArrowDown ]
        ; description = "arrow-down"
        ; group = None
        ; handler = (fun _ -> inject Game.Down)
        }
        ; { Command.keys = [ Keystroke.create' Js_of_ocaml.Dom_html.Keyboard_code.ArrowUp ]
        ; description = "arrow-up"
        ; group = None
        ; handler = (fun _ -> inject Game.Up)
        }
        ; { Command.keys = [ Keystroke.create' Js_of_ocaml.Dom_html.Keyboard_code.ArrowLeft ]
        ; description = "arrow-left"
        ; group = None
        ; handler = (fun _ -> inject Game.Left)
        }
        ; { Command.keys = [ Keystroke.create' Js_of_ocaml.Dom_html.Keyboard_code.ArrowRight ]
